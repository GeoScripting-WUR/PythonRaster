...
After creation, activate the environment, open Spyder, create a script in the root folder, and start coding.
...
Running the last line of code shows that the Web Coverage Service of the AHN3 contains four rasters: 0.5m DSM, 0.5m DTM, 5m DSM, and 5m DTM. Raster data of AHN has the projected coordinate system `RD_New (EPSG: 28992)`.
...
# Properly set our nodata to np.nan (this will be useful later)
...
The next step is to perform zonal statistics to get the average height value per building polygon. We will do this with the module Rasterstats, which can use a `GeoDataFrame` and a `.tif` file for this task. It outputs a [GeoJSON](http://geojson.org/).
...
If you do not like the orange colormap of Matplotlib, it is also possible to pick [another colormap](https://Matplotlib.org/examples/color/colormaps_reference.html).
