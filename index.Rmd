---
pagetitle: "Tutorial 10: Raster data handling with Python"
...
output:
  knitrBootstrap::bootstrap_document:
    title: "Tutorial 10: Raster data handling with Python"
    theme: "simplex"
...
Like in the previous lesson, we will create a conda environment with a `.yaml` file:
...
Use this file to create an environment for this tutorial. Activate the environment, open Spyder, create a script in the root folder and start coding.
...
Running the last line of code shows that the Web Coverage Service of the AHN2 contains four rasters: 0.5m DSM, 0.5m DTM, 5m DSM, and 5m DTM. The meters identify the cell size. Raster data of AHN has the projected coordinate system RD_New (EPSG: 28992).
...
Download the Digital Surface Model [DSM], which is the the 'ahn3_05m_dsm' version, and Digital Terrain Model [DTM], which is the 'ahn3_05m_dtm' version, to a local file. The difference between a DEM, DSM and DTM is explained on the [GIS StackExchange](https://gis.stackexchange.com/questions/5701/what-is-the-difference-between-dem-dsm-and-dtm/5704).
...
ds = gdal.Open("data/AHN3_05m_DTM.tif" )
...
figure(figsize=(5, 5), dpi=300)  # adjust size and resolution
...
```{block, type="alert alert-success"}
> **Question 2**: Is the colormap 'gist_ncar' cartographically a good choice for a digital surface model? Why/Why not? If not, try to find a more fitting one.
```
...
# Create a mask to specify which pixels to fill (0=fill, else=dont fill)
dtm_mask = dtm_data.copy()
dtm_mask[~np.isnan(dtm_data)] = 1
dtm_mask[np.isnan(dtm_data)] = 0

# Fill missing values
fillnodata(dtm_data, mask=dtm_mask)
...
```{block, type="alert alert-success"}
> **Question 3**: Where is the canopy the highest in the study area? Is it what you expected?
```

We have now applied the concepts of a Canopy Height Model. However, the area we picked does not only contain vegetation, but also buildings. 

## Computing heights of buildings

Let us determine the heights of buildings. First step is to download building data from the BAG Web Feature Service that we used in the vector tutorial.
...
```{block, type="alert alert-success"}
> **Question 4**: What happens in the line "bb = ','.join(map(str, bbox))"? Look up how mapping works if you do not know.
```
...
# More writing raster data to a file
